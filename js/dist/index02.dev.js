...
function fu_valorMulta() {
  ...
  var result_valorMulta = parseFloat(valor_boleto * inpvalor_multa / 100).toLocaleString("pt-BR", {
    style: 'currency',
    currency: 'BRL'
  });
...
function fun_valorJuros() {
  ...
  var diferençaDeTempo = Math.abs(inputDataPagamento - inputDataVencimento);
  var diferençaDeDias = Math.ceil(diferençaDeTempo / umDia);
  var result_valorJuros = Number(diferençaDeDias * inputValorMora).toFixed(4);
...
function fun_valorJurosEmdinheiro() {
  ...
  var diferençaDeTempo = Math.abs(inputDataPagamento - inputDataVencimento);
  var diferençaDeDias = Math.ceil(diferençaDeTempo / umDia);
  var valorJuros = parseFloat(diferençaDeDias * inputValorMora);
